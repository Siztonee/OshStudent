...
@section('content')
    <div class="container mx-auto py-8 px-4 dark:text-white text-red-600">
        <h1 class="text-3xl font-bold mb-6">Расписание занятий</h1>

        <div class="shadow-lg rounded-lg overflow-hidden dark:bg-gray-800 bg-gray-200">
            <div class="p-6">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full table-auto">
                        <thead>
                        <tr class="text-red-600 dark:text-gray-100 dark:bg-gray-700 bg-red-100">
                        ...
                        @foreach($days as $index => $day)
                            ...
                            @if($daySchedules->isNotEmpty())
                                @foreach($daySchedules as $schedule)
                                    <tr class="border-b">
                                        @if($loop->first)
                                            <td class="px-4 py-2 font-semibold dark:text-gray-300 text-red-500" rowspan="{{ $daySchedules->count() }}">
                                                {{ $day }}
                                            </td>
                                        @endif
                            ...
                            @else
                                <tr class="border-t-2 border-red-500 dark:border-red-700">
                                    <td class="px-4 py-2 font-semibold dark:text-gray-300 text-red-500">{{ $day }}</td>
